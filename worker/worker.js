...
export default {
	async fetch(request, env) {
		// Reject early if ALLOWED_ORIGIN is not configured
		...
		// Block requests not originating from the allowed frontend origin
		...
		try {
			...
			try {
				...
				return new Response(JSON.stringify({ error: 'AI step failed' }), {
					status: 500,
					headers: cors
				});
			}
